...
from model.timeline_container import TimelineContainer
from model.point_container import PointContainer
from ui.dynamic_ui import DynamicOkButton,DynamicSelectMenu
...
class messageListener(commands.Cog):
    ...
    async def on_message(self, message):
        try:
            ...
            if self.message.author.bot:
                ...
                await self.on_point_report_channels_message()
            elif self.message.channel.name == "タイムライン管理所":
                self.author_name = str(self.message.author.display_name)
                self.author_user_id = str(self.message.author)
                await self.on_timeline_report_channels_message()
            else:
                return
    ...
    async def on_point_report_channels_message(self):
        try:
    ...
    def analysis_point_spreadsheet(self):
        ...
        point_container = PointContainer(self.message.content)
        point_set_list = point_container.get_point_set_list()
        ...
        if (len(point_set_list) > 0):
            ...
            if (len(unupdated_list) == 0 and len(point_set_list) > 0):
                ...
                self.return_message += meow.meowmeow_accent(f"以下の内容でよいならOKをポチっと押してください\n",self.is_meow)
    ...
    async def on_timeline_report_channels_message(self):
        try:
            await self.message.add_reaction("🤔")
            self.is_meow = meow.meowmeow_check(str(self.message.content))
            ...
            self.return_message = f"{self.message.author.mention} "
            self.return_view = View()
            self.TimelineContainer = TimelineContainer(self.message, tl_author_other)
            if (self.TimelineContainer.message_type == "TL"):
                self.register_timeline_check()
                
            elif (self.TimelineContainer.message_type == "please"):
                self.please_timeline_check()

            elif (self.TimelineContainer.message_type == "delete"):
                self.delete_timeline_check()

            elif (self.TimelineContainer.message_type == "none"):
                if (self.is_meow):
                    self.return_message += meow.meowmeow_return()
    ...
    def register_timeline_check(self):
        ...
        self.return_message += self.TimelineContainer.tl_string
    ...
    def register_timeline(self):
        try:
            send_message = self.mongo_db.insert_tl(self.TimelineContainer.json_string)
            return send_message,View()
        except Exception as e:
            error_message = traceback.format_exc()
            return f"予期しないエラーが発生しました: {e}\n" + error_message,View()
    ...
    def please_timeline_check(self):
        element = self.TimelineContainer.search_type_dict["enemy_element"]
        level = self.TimelineContainer.search_type_dict["enemy_level"]
        point = self.TimelineContainer.search_type_dict["point"]
        attack_type = self.TimelineContainer.search_type_dict["attack_type"]
        author = self.TimelineContainer.search_type_dict["author"]
        see_all = self.TimelineContainer.search_type_dict["see_all"]
    ...
    def please_timeline(self,selected_value):
        ...
        send_message += self.TimelineContainer.tl_all_printer(document)
        return send_message,View()
